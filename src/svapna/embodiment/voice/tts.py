...
class PiperTTS:
    def __init__(self, model_dir: Path, voice: str, speaker_id: int | None = None,
        ...
        self._volume = volume
        self._syn_config = self._make_syn_config(speaker_id, volume)

    @staticmethod
    def _make_syn_config(speaker_id: int | None, volume: float):
        """Build a SynthesisConfig if speaker_id or non-default volume is set."""
        if speaker_id is None and volume == 1.0:
            return None
    ...
    async def synthesize(self, text: str) -> bytes:
        """Full-utterance WAV synthesis. Returns WAV bytes (PCM mono int16)."""
...
def resample_pcm_to_16k(pcm_bytes: bytes, src_rate: int) -> bytes:
    """Resample int16 mono PCM to 16kHz for the speaker_source path
    (the device's announcement_pipeline can take 16kHz mono and up-
    resample internally; sending at the speaker rate avoids two extra
    network bytes per sample)."""
    if src_rate == 16000:
        return pcm_bytes
    arr = np.frombuffer(pcm_bytes, dtype=np.int16).astype(np.float32)
    n_out = int(round(arr.size * 16000 / src_rate))
    # Simple linear interpolation. Piper voices are already band-limited
    # for their target rate so anti-alias filtering is not strictly
    # required for downsampling 22.05k -> 16k.
